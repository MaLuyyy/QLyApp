...
export default function AddProductModal({visible, onClose, onSuccess, staff }: Props){
    ...
    const handleEdit = async () => {
        ...
        try {
          ...
          const emailExists = staffs.some(
            (s: any) => s.email?.toLowerCase() === email.toLowerCase() && s.id !== s.id
          );
          const phoneExists = staffs.some(
            (s: any) => s.phone === phone && s.id !== s.id
          );
          ...
          if (!staff?.id) throw new Error("Thiếu ID món ăn");
          await updateDocument("staffs", staff.id, newStaff);
